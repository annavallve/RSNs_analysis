...
imagesc(mean_dif_AMP, [-0.1 0.1]); colormap (cmap); title ('Paired differences 50 ICs (AMPpost - AMPpre)', 'fontsize', 20)
set(gca, 'YTickLabel', '');
...
subplot (1,2,1); imagesc(reshape(mean_CORR_pre,N_goodnodes,N_goodnodes), [-1 1]); colormap (cmap); title (' Mean Correlation scan 1')
subplot (1,2,2); imagesc(reshape(mean_CORR_post,N_goodnodes,N_goodnodes), [-1 1]); colormap (cmap); title (' Mean Correlation scan 2')
...
figure ('Position', [0 0 1200 500], 'Color', 'white')
subplot (1,2,1); imagesc(reshape(mean_PCORR_pre,N_goodnodes,N_goodnodes), [-1 1]); colormap (cmap); title (' Mean Partial Correlation scan 1')
subplot (1,2,2); imagesc(reshape(mean_PCORR_post,N_goodnodes,N_goodnodes), [-1 1]); colormap (cmap); title (' Mean Partial Correlation scan 2')
...
figure ('Position', [0 0 1200 500], 'Color', 'white')
subplot (1,2,1); imagesc(reshape(mean_COV_pre,N_goodnodes,N_goodnodes), [-1 1]); colormap (cmap); title (' Mean Covariance time 1')
subplot (1,2,2); imagesc(reshape(mean_COV_post,N_goodnodes,N_goodnodes), [-1 1]); colormap (cmap); title (' Mean Covariance time 2')
